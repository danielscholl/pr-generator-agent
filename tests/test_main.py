...
@patch('aimr.main.generate_with_azure_openai')
@patch('aimr.main.git.Repo')
def test_main_with_vulnerability_workflow(mock_repo_class, mock_generate, mock_trivy_run, mock_repo, capsys):
...
def test_main_target_branch_fallback(mock_repo_class, mock_generate, mock_repo, capsys):
    ...
    captured = capsys.readouterr()
    assert f"{YELLOW}Warning: Target branch 'nonexistent' not found, using 'main' instead.{ENDC}" in captured.err
